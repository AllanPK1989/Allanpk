...
def readme(wb, fixed_offs):
    ...
    lines = [
        ...
        ("1. T1, T2 and E each get 2 consecutive days off a week (5 off blocks = 10 days in 28 Sep - 31 Oct).", False),
        ...
        ("6. Nobody works more than 6 days in a row (checked up to Sun 1 Nov); at least 3 days between offs.", False),
        ("", False),
        ...
        ("Offs are not tied to fixed weekdays: each person gets 2 days off after every 3-6 working days.", False),
        ("Exactly one supervisor covers each Sunday and the Sunday duty rotates between T1, T2 and E.", False),
        ("Everyone gets at least one full Sat + Sun weekend off.", False),
        ("Note: if offs had to fall inside each Mon-Sun week with a 6-day limit, E would have to work every Sunday.", False),
        ("", False),
